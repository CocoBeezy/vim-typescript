...
syntax region  tsString           start=+\z(["']\)+  skip=+\\\%(\z1\|$\)+  end=+\z1+ end=+$+  contains=tsSpecial extend
syntax region  tsTemplateString   start=+`+  skip=+\\`+  end=+`+     contains=tsTemplateExpression,tsSpecial extend
...
syntax region  tsObjectKeyString   contained start=+\z(["']\)+  skip=+\\\%(\z1\|$\)+  end=+\z1\|$+  contains=tsSpecial skipwhite skipempty nextgroup=tsObjectValue
...
syntax region  tsObjectStringKey   contained start=+\z(["']\)+  skip=+\\\%(\z1\|$\)+  end=+\z1\|$+  contains=tsSpecial extend skipwhite skipempty nextgroup=tsFuncArgs,tsObjectValue
...
syntax match   tsFuncName             contained /\<\K\k*/ skipwhite skipempty nextgroup=tsFuncArgs,tsFlowFunctionGroup
...
syntax region  tsClassStringKey         contained start=+\z(["']\)+  skip=+\\\%(\z1\|$\)+  end=+\z1\|$+  contains=tsSpecial extend skipwhite skipempty nextgroup=tsFuncArgs
...
" Comments
syntax keyword tsCommentTodo    contained TODO FIXME XXX TBD NOTE
...
if exists("typescript_plugin_tsdoc")
  runtime extras/tsdoc.vim
...
if exists("typescript_plugin_flow")
...
syntax cluster  htmlTypeScript       contains=@tsAll,tsImport,tsExport
syntax cluster  typeScriptExpression contains=@tsAll

" Vim's default html.vim highlights all typescript as 'Special'
